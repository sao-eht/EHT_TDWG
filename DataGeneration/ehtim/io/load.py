...
def load_movie_fits_hotspot(basename, nframes, nfiles, framedur, mjd=51544, start_hr=0., punit='deg', pulse=PULSE_DEFAULT):
	...
	current_frame += 1
        ...
        # Get the image and create the object, ORIG
        #data = hdulist[0].data
        ...
        data = hdulist[0].data
        image = data[0][::-1,:] # flip y-axis!
        framelist.append(image)

        # Look for Stokes Q and U, ORIG
        ...
        # Look for Stokes Q and U, MODIF
        qimage = uimage = vimage = np.array([])
        qimage = data[1][::-1,:] # CHECK, is this really Q?
        uimage = data[2][::-1,:] # CHECK, is this really U?
        qlist.append(qimage)
        ulist.append(uimage)

        hdulist.close()

    out_mov = ehtim.movie.Movie(framelist, framedur, psize0, ra0, dec0, rf=rf0, source=src0, mjd=mjd, start_hr=start_hr, pulse=pulse)

#    if len(qlist):
#        #print 'Loaded Stokes I, Q, and U movies'#Hotaka
#        out_mov.add_qu(qlist, ulist)
#    else:
#        print 'Loaded Stokes I movie only'
